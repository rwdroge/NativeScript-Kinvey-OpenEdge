...
const ItemDetailEditViewModel = require("./item-detail-edit-view-model");
...
function onNavigatingTo(args) {
    ...
    page.bindingContext = new ItemDetailEditViewModel(page.navigationContext);
}
...
function onDoneButtonTap(args) {
    ...
    bindingContext.saveChanges()
        .then(() => topmost().navigate({
            moduleName: "items/items-list-page",
            animated: true,
            clearHistory: true,
            transition: {
                name: "slideBottom",
...
function onSelectorTap(args) {
    ...
    const selectedValue = bindingContext.item[tag];
    const context = {
        tag,
        selectedValue
    };
    const modalPagePath = "items/list-selector-modal-page/list-selector-modal-page";
    const page = gridLayout.page;

    page.showModal(modalPagePath, context, (value) => {
        if (value) {
            bindingContext.item.set(tag, value);
        }
    }, false);
}
